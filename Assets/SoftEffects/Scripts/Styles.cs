...
namespace Mkey
{
    ...
    public class ShadowOptions : Options
    {
        ...
        public void SetCombinedOptions()
        {
            ...
            {
                CreateNoiseTexture(ref noiseTexture, new int2_mk(256, 256));
                noiseOld = noise;
            }
    ...
    public class FaceOptions : OptionsMain
    {
        ...
        private int2_mk[] inputUV;
        public int2_mk[] outputUV;
        ...
        internal void RenderFontTexture(GPUWorker gWorker, Font SoftFont, CBuffers cb)
        {
            ...
            inputUV = new int2_mk[ch.Length * 3];
            outputUV = new int2_mk[ch.Length * 3];
            ...
            gWorker.GPUCopyTextureUV(inputUV, outputUV, new int2_mk(image_width, image_height), fontTexture, ref r_mainTexture);
            //});
        ...
        internal void RenderSpriteTexture(GPUWorker gWorker, Texture inputTexture, CBuffers cb)
        {
            ...
            inputUV = new int2_mk[3];
            outputUV = new int2_mk[3];

            inputUV[0] = new int2_mk(0, 0);
            inputUV[1] = new int2_mk(tWidth, 0);
            inputUV[2] = new int2_mk(tWidth, tHeight);

            outputUV[0] = new int2_mk(extPixels, extPixels);
            outputUV[1] = new int2_mk(tWidth + extPixels, extPixels);
            outputUV[2] = new int2_mk(tWidth + extPixels, tHeight + extPixels);

            //9) render texture 
            gWorker.GPUCopyTextureUV(inputUV, outputUV, new int2_mk(image_width, image_height), inputTexture, ref r_mainTexture);
            outputUV[0] = new int2_mk(0, 0);
            outputUV[1] = new int2_mk(image_width, 0);
            outputUV[2] = new int2_mk(image_width, image_height);
            CreateUVMapBuffer(r_mainTexture.width, r_mainTexture.height, cb);
        }
    ...
    public class Options
    {
        ...
        public void CreateNoiseTexture(ref Texture2D output, int2_mk size)
        {
            int width = size.X;
            int height = size.Y;
